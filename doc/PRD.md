...
| content | String | AI 생성 피드백 텍스트 |
| createdAt | DateTime | 생성일시 |

### FeedbackKeyword (키워드)

Feedback에 속하는 키워드를 개별 행으로 관리한다. 키워드를 하나씩 추가/삭제할 수 있다.

| 필드 | 타입 | 설명 |
|------|------|------|
| id | Long | PK |
| feedbackId | Long | FK → Feedback |
| value | String | 키워드 텍스트 |
...
**POST /feedbacks/{feedbackId}/generate**
```json
// Response 200
{ "id": 1, "studentId": 1, "keywords": ["집중력 좋았음", "3단원 이해 부족"], "content": "안녕하세요...", "createdAt": "..." }
```

### 키워드 관리

키워드 추가 시 해당 학생의 피드백이 없으면 빈 피드백을 자동 생성한다.

| Method | URL | 설명 |
|--------|-----|------|
| POST | /students/{studentId}/keywords | 키워드 추가 (피드백 없으면 자동 생성) |
| DELETE | /feedbacks/{feedbackId}/keywords/{keywordId} | 키워드 삭제 |

**POST /students/{studentId}/keywords**
```json
// Request
{ "value": "집중력 좋았음" }

// Response 201
{ "id": 1, "feedbackId": 1, "value": "집중력 좋았음" }
```
