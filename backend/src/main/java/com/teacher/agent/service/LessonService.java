...
public class LessonService {
  ...
  public List<LessonResponse> getAllByTeacher(UserId userId) {
    Teacher teacher = findTeacherByUserId(userId);
    return lessonRepository.findAllByTeacherId(teacher.getId()).stream().map(LessonResponse::from)
        .toList();
  }
  ...
  private Lesson findById(Long id) {
    return lessonRepository.findById(id).orElseThrow(
        () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Lesson not found: " + id));
  }
